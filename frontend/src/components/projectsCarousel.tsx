...
export default function ProjectsCarousel({ projects, t, locale }: ProjectsCarouselProps) {
	...
	return (
		<section id="projects" className="my-10 md:mx-auto font-silkscreen relative w-full xl:w-10/12 bg-transparent">
			<motion.section
				ref={ref}
				...
				{/* horizontal slider */}
				   <div className="overflow-x-scroll flex gap-6 py-4 scrollbar-hide">
					{projects.map((project, index) => {
						return (
							<motion.div key={project._id} className="group shrink-0 w-72 h-56 bg-white rounded-xl transition duration-300 ease-linear hover:bg-deco text-deco">
								<a href={getProjectUrl(project.slug.current)} className="hover:text-white w-full h-full flex flex-col relative">
